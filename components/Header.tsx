    ...
    <>
      ...
      >
        <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-8">
          <div className="grid h-[84px] grid-cols-[auto_1fr_auto] items-center gap-4 sm:h-[96px] lg:h-[104px]">
            {/* Logo */}
            <Link href="/" className="group relative z-10 inline-flex items-center">
              <Image
            ...
            <nav className="hidden items-center justify-center gap-8 md:flex lg:gap-10">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="relative text-sm font-black uppercase tracking-[0.14em] text-black transition-colors hover:text-[#FF008E]"
                >
                  {item.label}
                </Link>
              ))}
